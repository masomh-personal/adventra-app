...
describe('getAllUserProfiles', () => {
  ...
  test('randomizes profile order', async () => {
    ...
    const originalOrder = mockData.map(item => item.user_id);
    const allSameOrder = results.every(result => {
      const resultOrder = result.map(item => item.user_id);
      return JSON.stringify(resultOrder) === JSON.stringify(originalOrder);
    });
