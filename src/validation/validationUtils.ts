...
export const passwordCriteria = [
    {
        ...
        yup: (schema: yup.StringSchema) =>
            schema.min(10, 'Password must be at least 10 characters'),
    },
    {
        label: 'Lowercase letter',
        test: (val: string | undefined): boolean => /[a-z]/.test(val ?? ''),
        yup: (schema: yup.StringSchema) =>
            schema.matches(/[a-z]/, 'Must include a lowercase letter'),
    },
    {
        label: 'Uppercase letter',
        test: (val: string | undefined): boolean => /[A-Z]/.test(val ?? ''),
        yup: (schema: yup.StringSchema) =>
            schema.matches(/[A-Z]/, 'Must include an uppercase letter'),
    },
    {
        label: 'Number',
